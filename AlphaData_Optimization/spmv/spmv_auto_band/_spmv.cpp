...
void ellpack(double* nzval, short* cols, double* vec, double* out)
{
#pragma HLS INLINE off
	...
	#pragma HLS UNROLL
						short tmp = cols[j + i*L];
            out[i] = out[i] + nzval[j + i*L] * vec[tmp];
        }
    }
}

void load_nzval(double* nzval, double local_nzval[UNROLL_FACTOR][L*ROWS_PER_TILE/UNROLL_FACTOR], int flag) {
#pragma HLS INLINE off
  int j;
  if (flag) {
      for (j=0; j<UNROLL_FACTOR; j++) {
          memcpy(local_nzval[j], nzval + j*ROWS_PER_TILE/UNROLL_FACTOR*L, sizeof(double)*ROWS_PER_TILE*L/UNROLL_FACTOR);
      }
  }
}

void load_cols(short* cols, short local_cols[UNROLL_FACTOR][L*ROWS_PER_TILE/UNROLL_FACTOR], int flag) {
#pragma HLS INLINE off
  int j;
  if (flag) {
      for (j=0; j<UNROLL_FACTOR; j++) {
...
void buffer_compute(double local_nzval[UNROLL_FACTOR][L*ROWS_PER_TILE/UNROLL_FACTOR], short local_cols[UNROLL_FACTOR][L*ROWS_PER_TILE/UNROLL_FACTOR], 
		    double local_vec[UNROLL_FACTOR][N], double local_out[UNROLL_FACTOR][ROWS_PER_TILE/UNROLL_FACTOR], int flag, double* out) {
#pragma HLS INLINE off
  int j;
  if (flag) {
    for (j=0; j<UNROLL_FACTOR; j++) {
    ...
    for (j=0; j<UNROLL_FACTOR; j++) {
	memcpy(out + j*ROWS_PER_TILE/UNROLL_FACTOR, local_out[j], sizeof(double)*ROWS_PER_TILE/UNROLL_FACTOR);
    }
  }
}


void workload(double* nzval, short* cols, double* vec, double* out) {
...
#pragma HLS INTERFACE s_axilite port=return bundle=control
	...
	double local_nzval_x[UNROLL_FACTOR][L*ROWS_PER_TILE/UNROLL_FACTOR];
	...
	double local_nzval_y[UNROLL_FACTOR][L*ROWS_PER_TILE/UNROLL_FACTOR];
	...
	double local_vec[UNROLL_FACTOR][N];
	#pragma HLS ARRAY_PARTITION variable=local_vec dim=1 complete
	memcpy(local_vec[0], vec, N*sizeof(double));
	for(i=0; i<N; i++) {
        #pragma HLS PIPELINE
	...
	double local_out[UNROLL_FACTOR][ROWS_PER_TILE/UNROLL_FACTOR];
